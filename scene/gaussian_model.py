...
class GaussianModel:
    ...
    def get_fake_features(self):
        ...
        features_dc = self._features_dc
        return torch.cat((features_dc, features_fake), dim=1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fake_features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        ...
        #test_tag add fake color shs
        self._features_fake = nn.Parameter(fake_features.transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._features_fake.shape[1]*self._features_fake.shape[2]):
            l.append('f_fake_{}'.format(i))
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        f_fake = self._features_fake.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def load_ply(self, path):
        ...
        fake_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_fake_")]
        fake_f_names = sorted(fake_f_names, key = lambda x: int(x.split('_')[-1]))
        assert len(fake_f_names)==3*(self.max_sh_degree + 1) ** 2
        features_fake = np.zeros((xyz.shape[0], len(fake_f_names)))
        for idx, attr_name in enumerate(fake_f_names):
            features_fake[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_fake = features_fake.reshape((features_fake.shape[0], 3, (self.max_sh_degree + 1) ** 2))
